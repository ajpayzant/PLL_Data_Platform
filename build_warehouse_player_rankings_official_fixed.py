...
def _add_player_ranking_scores(df):
    """
    Build the official Colab-tested player ranking scores.

    This intentionally uses the simpler final test system:
      - min-max component scores inside each ranking context
      - offense / usage / defense / faceoff / goalie component scores
      - role primary score, role percentile, and peer-separation score
      - role context value score
      - official overall_score used by the Streamlit app

    The app should display these fields directly and should not recalculate
    the official score.
    """
    out = df.copy()
    ...
    # Useful efficiency/helper fields retained from the test mart contract.
    ...
    # ------------------------------------------------------------
    # Component scores — same structure as the final Colab test.
    # ------------------------------------------------------------
    ...
    out["one_point_goal_score"] = _minmax_score(out["one_point_goals_per_game"], True)
    out["two_point_goal_score"] = _minmax_score(out["two_point_goals_per_game"], True)
    out["scoring_points_score"] = _minmax_score(out["scoring_points_per_game"], True)
    ...
    out["ground_ball_score"] = _minmax_score(out["ground_balls_per_game"], True).fillna(50)

    # ------------------------------------------------------------
    # Role context — score + percentile + separation.
    # ------------------------------------------------------------
    ...
    for _, idx in out.groupby("role_group", dropna=False).groups.items():
        ...
        # Shrink peer-separation signal when the role group is tiny.
        n = len(idx)
        ...
        out.loc[idx, "role_reliability"] = shrink * 100.0
    ...
    for _, idx in out.groupby("role_group", dropna=False).groups.items():
        ...
        if valid.sum() > 0:
            out.loc[valid, "role_context_rank"] = out.loc[valid, "role_context_value_score"].rank(method="min", ascending=False)
            out.loc[valid, "role_context_percentile"] = _rank_pct(out.loc[valid, "role_context_value_score"], True).values

    # ------------------------------------------------------------
    # Base impact and official overall score.
    # ------------------------------------------------------------
    ...
    # The Colab audit/test treated overall_impact_score as the visible player rating.
    # Keep it aligned with the official visible score for one-column consistency.
