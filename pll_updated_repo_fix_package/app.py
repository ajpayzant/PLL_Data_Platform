...
REPO_ROOT = APP_FILE.parent
DATA_ROOT = REPO_ROOT / "data"

DB_PATH = str(DATA_ROOT / "analytics_database" / "pll_warehouse.duckdb")
ARTIFACT_INDEX_PATH = str(DATA_ROOT / "curated_data" / "all_requested_seasons" / "artifact_index.csv")
...
def filter_values():
    """
    Load shared filter values using schema-safe pandas normalization.

    The Colab warehouse and GitHub warehouse can expose player/team directory
    fields with slightly different names, such as latest_team_id/latest_team_name
    instead of team_id/team_name. This keeps the app layout unchanged while
    preventing DuckDB BinderException errors from optional columns.
    """

    def _read_table_safe(schema_table):
        try:
            return query_df(f"SELECT * FROM {schema_table}")
        except Exception:
            return pd.DataFrame()

    # Seasons
    schedule_df = _read_table_safe("clean.game_schedule_all")
    if len(schedule_df) > 0 and "season" in schedule_df.columns:
        seasons = (
            pd.to_numeric(schedule_df["season"], errors="coerce")
            .dropna()
            .astype(int)
            .drop_duplicates()
            .sort_values()
    ...
    # Teams
    teams_raw = _read_table_safe("clean.team_directory")
    ...
    if len(teams_raw) == 0:
        ...
        if "team_name" not in teams.columns:
            for candidate in ["latest_team_name", "official_team_name", "current_team_name", "name", "team_display_name"]:
                if candidate in teams.columns:
    ...
    # Players
    players_raw = _read_table_safe("clean.player_directory")
    ...
    if len(players_raw) == 0:
        ...
        if "full_name" not in players.columns:
            for candidate in ["player_name", "name", "display_name"]:
                if candidate in players.columns:
                    players["full_name"] = players[candidate]
                    break

        if "full_name" not in players.columns:
            players["full_name"] = players.get("player_id", pd.Series(["Unknown"] * len(players)))

        if "position" not in players.columns:
            for candidate in ["primary_position", "position_name"]:
                if candidate in players.columns:
        ...
        if "team_id" not in players.columns:
            for candidate in ["latest_team_id", "official_team_id", "current_team_id", "team_abbrev"]:
                if candidate in players.columns:
        ...
        if "team_name" not in players.columns:
            for candidate in ["latest_team_name", "official_team_name", "current_team_name"]:
                if candidate in players.columns:
        ...
        if "player_id" not in players.columns:
            players["player_id"] = (
                players["full_name"].astype(str).str.lower().str.replace(r"[^a-z0-9]+", "_", regex=True).str.strip("_")
            )
    ...
    # Positions
    player_games = _read_table_safe("clean.player_game_stats")
    if len(player_games) > 0 and "position" in player_games.columns:
        positions = (
            player_games["position"]
            .dropna()
            .astype(str)
            .replace("", np.nan)
            .dropna()
    ...
    elif len(players) > 0 and "position" in players.columns:
        positions = (
            players["position"]
            .dropna()
            .astype(str)
...
with tab_matchup:
    ...
    if len(matchup_pool) == 0:
        ...
        matchup_matches = matchup_pool[matchup_pool["matchup_label"] == selected_matchup_label]

        if len(matchup_matches) == 0:
            matchup = matchup_pool.iloc[0]
        else:
            matchup = matchup_matches.iloc[0]
