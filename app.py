...
def filter_values():
    """
    App filter values, loaded in a schema-safe way.

    This preserves the Colab UI/output while allowing the GitHub warehouse to
    use either the final Colab directory schema or a deployment-safe fallback
    schema (for example latest_team_id/latest_team_name).
    """
    try:
        schedule_df = query_df("""
            ...
            ORDER BY season
        """)
        seasons = schedule_df["season"].dropna().astype(int).tolist()
    except Exception:
        try:
            schedule_df = read_table("clean.game_schedule_all")
            seasons = (
                pd.to_numeric(schedule_df.get("season", pd.Series(dtype="float")), errors="coerce")
                .dropna()
                .astype(int)
                .drop_duplicates()
                .sort_values()
    ...
    if teams is None or len(teams) == 0:
        ...
        if "team_id" not in teams.columns:
            for c in ["latest_team_id", "official_team_id", "team_abbrev", "team"]:
                if c in teams.columns:
                    teams["team_id"] = teams[c]
                    break
        if "team_name" not in teams.columns:
            for c in ["latest_team_name", "official_team_name", "team_display_name", "name"]:
                if c in teams.columns:
    ...
    if players is None or len(players) == 0:
        ...
        if "full_name" not in players.columns:
            for c in ["player_name", "name", "display_name"]:
        ...
        if "position" not in players.columns:
            for c in ["position_name", "primary_position"]:
                if c in players.columns:
        ...
        if "team_id" not in players.columns:
            for c in ["latest_team_id", "official_team_id", "current_team_id", "team_abbrev"]:
                if c in players.columns:
        ...
        if "team_name" not in players.columns:
            for c in ["latest_team_name", "official_team_name", "current_team_name"]:
                if c in players.columns:
        ...
        if "player_id" not in players.columns:
            players["player_id"] = players["full_name"].astype(str).str.lower().str.replace(r"[^a-z0-9]+", "_", regex=True)
    ...
    try:
        positions = query_df("""
            ...
            ORDER BY position
        """)["position"].dropna().tolist()
    except Exception:
        if players is not None and len(players) > 0 and "position" in players.columns:
            positions = sorted(players["position"].dropna().astype(str).unique().tolist())
        else:
            positions = []
