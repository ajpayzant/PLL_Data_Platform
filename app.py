...
COL_LABELS.update({
    ...
    "offensive_creation_score": "Scoring + Playmaking",
...
COL_LABELS.update({
    ...
    "playmaking_value_score": "Playmaking Value",
    "offensive_creation_score": "Scoring + Playmaking",
    "one_point_goal_score": "1PT Scoring Value",
    "two_point_goal_score": "2PT Scoring Value",
...
def _pll_prepare_player_rankings(rankings):
    ...
    # Scoring Value is the final label for the older goal_value_score concept.
...
with tab_player_rankings:
    ...
    st.markdown(
        '<div class="section-note">Rank players using the official overall score, which combines production, role value, usage, scoring value, playmaking value, and peer separation.</div>',
        unsafe_allow_html=True
    )
    ...
    if len(rankings) == 0:
        ...
        with st.expander("Ranking Method", expanded=False):
            st.markdown(
                ...
                **Scoring Value** captures direct scoring impact: scoring points, goals, 1PT/2PT goal value, and 2PT scoring efficiency.

                **Playmaking Value** captures creation impact: assists, assists per touch, points per touch, passing involvement, and turnover security.

                **Overall Score**
                - **Offense:** 60% Base Impact + 20% Role Context + 10% Usage + 10% Scoring/Playmaking Creation
        ...
        # For the Overall table, keep all field-area columns visible, but blank
        # specialist-only stats for players who do not play that role. This keeps
        # the all-player view readable without hiding the broader stat categories.
        ranking_table_df = filtered_rankings[ranking_display_cols].copy()
        if ranking_view == "Overall" and "role_group" in filtered_rankings.columns:
            specialist_blank_rules = {
            ...
            for role_name, cols_to_blank in specialist_blank_rules.items():
                not_role_mask = ~filtered_rankings["role_group"].astype(str).eq(role_name)
            ...
            if len(player_detail):
                ...
                breakdown_df = pd.DataFrame({
                    ...
                    "score": [
                        ...
                        row.get("scoring_value_score", np.nan),
...
with tab_dictionary:
    ...
    with guide_tabs[2]:
        ...
        ranking_defs = pd.DataFrame([
            ...
            ["Scoring/Playmaking Creation", "Offensive creation blend used in the final offense formula: 60% Scoring Value and 40% Playmaking Value."],
