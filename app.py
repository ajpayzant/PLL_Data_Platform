...
def _pll_build_v22_player_rankings(rankings):
    """
    Official player ranking display layer.

    Production rule:
    - If the warehouse mart is marked as the Colab-tested official ranking mart,
      preserve the mart-built Overall Score / Overall Rank exactly.
    - Otherwise, fall back to the same role-context calculation used during testing.

    This prevents Streamlit from double-scoring a mart that already contains the
    official Colab-tested ranking output.
    """
    ...
    # --------------------------------------------------------
    # Normalize role group
    # --------------------------------------------------------
    if "role_group" not in df.columns:
        pos = df.get("position", pd.Series("", index=df.index)).astype(str).str.upper().str.strip()
        df["role_group"] = np.where(
            ...
            np.where(
                pos.isin(["FO", "FOS"]),
                "Faceoff",
                np.where(pos.isin(["D", "LSM", "SSDM"]), "Defense", "Offense")
            )
        )

    # --------------------------------------------------------
    # If builder already produced the official Colab-tested mart,
    # mirror the official mart columns into the v22 display aliases.
    # --------------------------------------------------------
    locked = pd.Series(False, index=df.index)

    if "official_score_locked" in df.columns:
        locked = locked | pd.to_numeric(df["official_score_locked"], errors="coerce").fillna(0).eq(1)

    if "ranking_formula_version" in df.columns:
        locked = locked | df["ranking_formula_version"].astype(str).str.contains(
            "colab_test_mart_official|colab_official_player_ranking",
            case=False,
            na=False,
            regex=True,
        )

    # Use the exact official mart columns when they are available.
    if locked.any():
        score_source = None
        for candidate in ["overall_score", "official_overall_score", "overall_impact_score", "v22_overall_score"]:
            if candidate in df.columns and pd.to_numeric(df[candidate], errors="coerce").notna().any():
                score_source = candidate
                break

        rank_source = None
        for candidate in ["overall_rank", "official_overall_rank", "v22_overall_rank"]:
            if candidate in df.columns and pd.to_numeric(df[candidate], errors="coerce").notna().any():
                rank_source = candidate
                break

        position_rank_source = None
        for candidate in ["position_rank", "official_position_rank", "v22_position_rank"]:
            if candidate in df.columns and pd.to_numeric(df[candidate], errors="coerce").notna().any():
                position_rank_source = candidate
                break

        if score_source is not None:
            df["v22_overall_score"] = pd.to_numeric(df[score_source], errors="coerce")
            df["overall_score"] = df["v22_overall_score"]
            df["official_overall_score"] = df["v22_overall_score"]
            if "base_impact_score" not in df.columns:
                df["base_impact_score"] = pd.to_numeric(df.get("overall_impact_score", df["v22_overall_score"]), errors="coerce")

        if rank_source is not None:
            df["v22_overall_rank"] = pd.to_numeric(df[rank_source], errors="coerce")
            df["overall_rank"] = df["v22_overall_rank"]

        if position_rank_source is not None:
            df["v22_position_rank"] = pd.to_numeric(df[position_rank_source], errors="coerce")
            df["position_rank"] = df["v22_position_rank"]

        # Preserve / create percentiles if absent.
        if "v22_overall_percentile" not in df.columns or df["v22_overall_percentile"].isna().all():
            df["v22_overall_percentile"] = np.nan
            score_col = "v22_overall_score"
            eligible_mask = (
                df.get("eligible_for_default_ranking", pd.Series(True, index=df.index))
                .fillna(True)
                .astype(bool)
            )
            for _, idx in df.groupby("ranking_context", dropna=False).groups.items():
                idx = list(idx)
                valid = df.index.isin(idx) & eligible_mask & pd.to_numeric(df[score_col], errors="coerce").notna()
                if valid.sum() > 0:
                    df.loc[valid, "v22_overall_percentile"] = (
                        pd.to_numeric(df.loc[valid, score_col], errors="coerce")
                        .rank(ascending=True, pct=True, method="average") * 100
                    )

        if "v22_position_percentile" not in df.columns or df["v22_position_percentile"].isna().all():
            ...
            for _, idx in df.groupby(["ranking_context", "position"], dropna=False).groups.items():
                idx = list(idx)
                valid = df.index.isin(idx) & eligible_mask & pd.to_numeric(df["v22_overall_score"], errors="coerce").notna()
                if valid.sum() > 0:
                    df.loc[valid, "v22_position_percentile"] = (
                        pd.to_numeric(df.loc[valid, "v22_overall_score"], errors="coerce")
                        .rank(ascending=True, pct=True, method="average") * 100
                    )
    ...
    # --------------------------------------------------------
    # Fallback: calculate official score from Colab-tested fields.
    # --------------------------------------------------------
        ...
        for _, idx in df.groupby(["ranking_context", "position"]).groups.items():
            ...
            if valid.sum() > 0:
                df.loc[valid, "v22_position_rank"] = (
                    df.loc[valid, "v22_overall_score"].rank(ascending=False, method="min")
                )
                df.loc[valid, "v22_position_percentile"] = (
                    df.loc[valid, "v22_overall_score"].rank(ascending=True, pct=True, method="average") * 100
                )
