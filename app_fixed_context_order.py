...
DB_PATH = str(PROJECT_ROOT / "analytics_database" / "pll_warehouse.duckdb")
ARTIFACT_INDEX_PATH = str(PROJECT_ROOT / "curated_data" / "all_requested_seasons" / "artifact_index.csv")
...
def filter_values():
    """
    Return global filter values without assuming one exact player/team directory schema.

    The Colab-tested warehouse and GitHub warehouse can differ slightly in directory
    field names. In particular, clean.player_directory may contain latest_team_id /
    latest_team_name instead of team_id / team_name. Reading the directory table first
    and normalizing in pandas keeps the final app design identical while avoiding
    DuckDB BinderException crashes when optional columns are absent.
    """
    ...
    try:
        ...
        if len(seasons_df) > 0 and "season" in seasons_df.columns:
            seasons = (
                pd.to_numeric(seasons_df["season"], errors="coerce")
                .dropna()
                .astype(int)
                .tolist()
            )
    ...
    # ------------------------------------------------------------
    try:
        teams_raw = read_table("clean.team_directory")
    except Exception:
        teams_raw = pd.DataFrame()

    if teams_raw is None or len(teams_raw) == 0:
        teams = pd.DataFrame(columns=["team_id", "team_name"])
    else:
        teams = teams_raw.copy()

        if "team_id" not in teams.columns:
            for candidate in ["latest_team_id", "official_team_id", "team_abbrev", "team", "id"]:
                if candidate in teams.columns:
                    teams["team_id"] = teams[candidate]
                    break

        if "team_name" not in teams.columns:
            for candidate in ["latest_team_name", "official_team_name", "name", "team_display_name", "display_name"]:
                if candidate in teams.columns:
                    teams["team_name"] = teams[candidate]
                    break
    ...
    # ------------------------------------------------------------
    try:
        players_raw = read_table("clean.player_directory")
    except Exception:
        players_raw = pd.DataFrame()

    if players_raw is None or len(players_raw) == 0:
        ...
        if "player_id" not in players.columns:
            for candidate in ["id", "official_player_id", "playerId", "athlete_id"]:
                if candidate in players.columns:
                    players["player_id"] = players[candidate]
                    break

        if "full_name" not in players.columns:
            for candidate in ["player_name", "name", "display_name", "latest_full_name"]:
                if candidate in players.columns:
                    players["full_name"] = players[candidate]
                    break

        if "position" not in players.columns:
            for candidate in ["latest_position", "primary_position", "pos"]:
                if candidate in players.columns:
                    players["position"] = players[candidate]
                    break

        if "position_name" not in players.columns:
            for candidate in ["latest_position_name", "primary_position_name", "position"]:
                if candidate in players.columns:
                    players["position_name"] = players[candidate]
                    break

        if "team_id" not in players.columns:
            for candidate in ["latest_team_id", "official_team_id", "current_team_id", "team_abbrev", "team"]:
                if candidate in players.columns:
                    players["team_id"] = players[candidate]
                    break

        if "team_name" not in players.columns:
            for candidate in ["latest_team_name", "official_team_name", "current_team_name", "team_display_name"]:
                if candidate in players.columns:
                    players["team_name"] = players[candidate]
                    break

        if "player_id" not in players.columns:
            if "normalized_name" in players.columns:
                players["player_id"] = players["normalized_name"]
            elif "full_name" in players.columns:
                players["player_id"] = (
        ...
        keep_cols = ["player_id", "full_name", "position", "position_name", "team_id", "team_name"]
    ...
    if players is not None and len(players) > 0 and "position" in players.columns:
        ...
        try:
            positions_df = query_df("""
                ...
                ORDER BY position
            """)
            positions = positions_df["position"].dropna().tolist()
        except Exception:
            positions = []
...
with tab_matchup:
    ...
    if len(matchup_pool) == 0:
        ...
        # Select by row index rather than label lookup to avoid stale Streamlit widget
        # values causing an empty match and .iloc[0] crash after filters change.
        selected_matchup_idx = st.selectbox(
            "Select game",
            options=list(matchup_pool.index),
            index=0,
            format_func=lambda i: str(matchup_pool.loc[i, "matchup_label"]),
            key="selected_matchup_label"
        )

        matchup = matchup_pool.loc[selected_matchup_idx]
...
def _pll_context_order(df, context_col, type_col, sort_col):
    """
    Return ordered context labels for ranking/profile selectboxes.

    The Colab app expected explicit sort columns such as:
        - ranking_context_sort
        - profile_context_sort

    Some GitHub-built warehouses do not include those columns. This function
    keeps the same UI behavior but safely derives a sort value from the context
    label when the explicit sort column is missing. This prevents the deployed
    app from crashing while preserving the tested app layout/output.
    """
    ...
    if type_col not in out.columns:
        out[type_col] = np.where(
            out[context_col].astype(str).str.contains("Career", case=False, na=False),
            "Career",
            np.where(
                out[context_col].astype(str).str.contains("Last", case=False, na=False),
                "Recent",
                "Season",
            ),
        )
    ...
    if sort_col not in out.columns:
        context_text = out[context_col].astype(str)
        year_sort = context_text.str.extract(r"(20\d{2})", expand=False)
        last_sort = context_text.str.extract(r"Last\s*(\d+)", expand=False)

        # Use large values for season years, smaller values for last-N splits,
        # and 0 as the fallback. Career is handled by _type_order below.
        out[sort_col] = pd.to_numeric(year_sort, errors="coerce")
        out[sort_col] = out[sort_col].where(
            out[sort_col].notna(),
            pd.to_numeric(last_sort, errors="coerce"),
        )
        out[sort_col] = out[sort_col].fillna(0)

    out = out[[context_col, type_col, sort_col]].drop_duplicates().copy()

    type_text = out[type_col].astype(str)
    context_text = out[context_col].astype(str)

    # Keep Career first, then season contexts newest-to-oldest, then recent
    # Last 10 / Last 5 style contexts, then anything else.
    out["_type_order"] = np.select(
        [
            type_text.eq("Career") | context_text.eq("Career"),
            type_text.eq("Season") | context_text.str.contains("Season", case=False, na=False),
            type_text.str.contains("Last|Recent", case=False, na=False) | context_text.str.contains("Last", case=False, na=False),
        ],
        [0, 1, 2],
        default=3,
    )
    out["_sort"] = pd.to_numeric(out[sort_col], errors="coerce").fillna(0)
    ...
    return out[context_col].dropna().astype(str).tolist()
