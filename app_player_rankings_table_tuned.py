...
def _pll_prepare_player_rankings(rankings):
    ...
    for col in [
        ...
        "goal_value_score", "ground_ball_score", "base_impact_score",
    ]:
        if col not in df.columns:
            df[col] = np.nan
        ...
        with st.expander("Ranking Method", expanded=False):
            st.markdown(
                ...
                **Overall Score**
                ...
                - **Goalie:** 68% Base Impact + 24% Role Context + 5% Save Percentage + 3% Save Volume, with a small overall-table moderation
