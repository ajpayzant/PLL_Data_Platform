...
def _add_player_ranking_scores(df):
    ...
    offense_base = _weighted_available_score(out, {
        "offensive_score": 0.56,
        "usage_possession_score": 0.22,
        "goal_value_score": 0.14,
        "ground_ball_score": 0.08,
    }, fallback=50)
    ...
    #
    # This keeps the tested role-specific architecture, but uses a middle-ground
    # faceoff formula so FO specialists still win the Faceoff view without being
    # pushed too aggressively into the overall top tier on faceoff dominance
    # alone.
    # ------------------------------------------------------------------
    out["overall_score_raw"] = np.select(
        ...
        [
            ...
            # Goalies remain rated by goalie-specific value, but the overall
            # cross-position formula is intentionally less role-context heavy
            # than the dedicated Goalie view. This prevents small-pool goalie
            # dominance from crowding the top of career/overall rankings.
            0.70 * out["base_impact_score"] + 0.14 * out["role_context_value_score"] + 0.09 * out["save_pct_score"] + 0.07 * out["saves_score"],
        ],
    ...
    # impact, while goalies should be less aggressive in the all-player overall
    # table because they are evaluated inside a much smaller, specialized peer pool.
    out["role_overall_adjustment"] = 0.0
    out.loc[out["role_group"].eq("Defense"), "role_overall_adjustment"] = 1.5
    out.loc[out["role_group"].eq("Goalie"), "role_overall_adjustment"] = -7.0
