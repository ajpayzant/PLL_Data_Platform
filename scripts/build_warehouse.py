...
def _add_player_ranking_scores(df):
    ...
    out["overall_score_raw"] = np.select(
        ...
        [
            ...
            0.68 * out["base_impact_score"] + 0.24 * out["role_context_value_score"] + 0.05 * out["save_pct_score"] + 0.03 * out["saves_score"],
        ],
    ...
    # impact, while goalies should be slightly less aggressive in the all-player
    # overall table.
    out["role_overall_adjustment"] = 0.0
    out.loc[out["role_group"].eq("Defense"), "role_overall_adjustment"] = 1.5
    out.loc[out["role_group"].eq("Goalie"), "role_overall_adjustment"] = -2.5
