...
# PLAYER RANKING CONTRACT: Colab-tested official mart
# ============================================================
...
def _coalesce_existing_numeric(df, target_col, candidates):
    if target_col not in df.columns:
        df[target_col] = np.nan
    df[target_col] = pd.to_numeric(df[target_col], errors="coerce")
    for c in candidates:
        if c in df.columns:
            df[target_col] = df[target_col].combine_first(pd.to_numeric(df[c], errors="coerce"))
    return df
...
def _weighted_row_score(df, weights, fallback=np.nan):
...
def _add_player_ranking_scores(df):
    """
    Build the Colab-tested player ranking mart.

    Important contract:
    - overall_impact_score / overall_score are the official displayed score.
    - overall_rank / position_rank are the official displayed ranks.
    - v22_* columns are aliases only so the current Streamlit page keeps working.
    - app.py should not double-score rows marked official_score_locked = 1.
    """
    out = df.copy()
    ...
    needed = [
        ...
        "shots_per_game", "shots_on_goal_per_game", "shot_pct_calc", "shot_pct",
        "ground_balls_per_game", "turnovers_per_game", "caused_turnovers_per_game",
        "touches_per_game", "total_passes_per_game", "faceoff_pct_calc", "faceoff_pct",
        ...
        "save_pct_calc", "save_pct", "two_point_shots", "two_point_goals",
        "games"
    ]
    out = _ensure_cols(out, needed)

    out = _coalesce_existing_numeric(out, "faceoff_pct_calc", ["faceoff_pct"])
    out = _coalesce_existing_numeric(out, "save_pct_calc", ["save_pct"])
    out = _coalesce_existing_numeric(out, "shot_pct_calc", ["shot_pct"])
    ...
    def pct(col, higher_is_better=True):
        return _rank_pct(out[col], higher_is_better=higher_is_better) if col in out.columns else pd.Series(np.nan, index=out.index)

    # ------------------------------------------------------------
    # Context percentiles. This matches the tested ranking behavior
    # better than min-max scoring and prevents specialist inflation.
    # ------------------------------------------------------------
    out["points_score"] = pct("points_per_game", True)
    out["scoring_points_score"] = pct("scoring_points_per_game", True)
    out["one_point_goal_score"] = pct("one_point_goals_per_game", True)
    out["two_point_goal_score"] = pct("two_point_goals_per_game", True)
    out["goals_score"] = pct("goals_per_game", True)
    out["assists_score"] = pct("assists_per_game", True)
    out["shots_score"] = pct("shots_per_game", True)
    out["shots_on_goal_score"] = pct("shots_on_goal_per_game", True)
    out["shot_pct_score"] = pct("shot_pct_calc", True)
    out["touches_score"] = pct("touches_per_game", True)
    out["passes_score"] = pct("total_passes_per_game", True)
    out["ground_ball_score"] = pct("ground_balls_per_game", True)
    out["caused_turnover_score"] = pct("caused_turnovers_per_game", True)
    out["turnover_security_score"] = pct("turnovers_per_game", False)
    out["faceoff_pct_score"] = pct("faceoff_pct_calc", True)
    out["faceoff_volume_score"] = pct("faceoffs_per_game", True)
    out["faceoff_win_score"] = pct("faceoffs_won_per_game", True)
    out["save_pct_score"] = pct("save_pct_calc", True)
    out["saves_score"] = pct("saves_per_game", True)
    out["goals_against_score"] = pct("goals_against_per_game", False)
    out["scores_against_score"] = pct("scores_against_per_game", False)

    out["points_per_touch"] = np.where(
        pd.to_numeric(out["touches_per_game"], errors="coerce") > 0,
        pd.to_numeric(out["points_per_game"], errors="coerce") / pd.to_numeric(out["touches_per_game"], errors="coerce"),
        np.nan,
    )
    out["assists_per_touch"] = np.where(
        pd.to_numeric(out["touches_per_game"], errors="coerce") > 0,
        pd.to_numeric(out["assists_per_game"], errors="coerce") / pd.to_numeric(out["touches_per_game"], errors="coerce"),
        np.nan,
    )

    out["points_per_touch_score"] = _rank_pct(out["points_per_touch"], True)
    out["assists_per_touch_score"] = _rank_pct(out["assists_per_touch"], True)
    ...
    if {"two_point_shots", "two_point_goals"}.issubset(out.columns):
        ...
        out["two_point_goal_efficiency_score"] = _rank_pct(out["two_point_goal_pct_calc"], True)
    else:
        out["two_point_goal_efficiency_score"] = np.nan

    out["goal_value_score"] = _weighted_row_score(out, {
        "scoring_points_score": 0.30,
        "points_score": 0.22,
        "one_point_goal_score": 0.16,
        "two_point_goal_score": 0.22,
        "two_point_goal_efficiency_score": 0.10,
    }, fallback=np.nan)

    out["offensive_score"] = _weighted_row_score(out, {
        "points_score": 0.28,
        "scoring_points_score": 0.22,
        "goals_score": 0.14,
        "assists_score": 0.14,
        "shots_score": 0.08,
        "goal_value_score": 0.10,
        "points_per_touch_score": 0.04,
    }, fallback=np.nan)

    out["usage_possession_score"] = _weighted_row_score(out, {
        "touches_score": 0.42,
        "shots_score": 0.20,
        "passes_score": 0.14,
        "ground_ball_score": 0.12,
        "shots_on_goal_score": 0.06,
        "assists_per_touch_score": 0.06,
    }, fallback=np.nan)
    out["usage_score"] = out["usage_possession_score"]

    out["defensive_score"] = _weighted_row_score(out, {
        "caused_turnover_score": 0.48,
        "ground_ball_score": 0.32,
        "turnover_security_score": 0.14,
        "touches_score": 0.06,
    }, fallback=np.nan)

    out["faceoff_score"] = _weighted_row_score(out, {
        "faceoff_pct_score": 0.58,
        "faceoff_win_score": 0.22,
        "ground_ball_score": 0.14,
        "faceoff_volume_score": 0.06,
    }, fallback=np.nan)

    out["goalie_score"] = _weighted_row_score(out, {
        "save_pct_score": 0.60,
        "saves_score": 0.22,
    ...
    # Role primary is the comparable peer score for each role.
    role_score_col = {
    ...
    for role_name, col in role_score_col.items():
    ...
    out["role_context_value_score"] = _weighted_row_score(out, {
    ...
    # ------------------------------------------------------------
    # Official Colab-style overall score.
    # These are the same public names used by the test mart/app.
    # v22_* is only a compatibility alias for the existing UI.
    # ------------------------------------------------------------
    ...
    out["overall_impact_score"] = pd.to_numeric(out["overall_impact_score"], errors="coerce").clip(0, 100)
    out["base_impact_score"] = out["overall_impact_score"]
    out["overall_score"] = out["overall_impact_score"]
    out["official_overall_score"] = out["overall_impact_score"]
    out["v22_overall_score"] = out["overall_impact_score"]

    out["ranking_formula_version"] = "colab_test_mart_official"
...
def _build_player_ranking_context(df, context_type, context_label, sort_order):
    ...
    out["is_ranking_eligible"] = (out["games"] >= min_games).astype(int)
    out["eligible_for_default_ranking"] = out["is_ranking_eligible"].astype(bool)
    out["sample_size_note"] = np.where(max_games <= 2, "Early-season sample.", "")

    out = _add_player_ranking_scores(out)

    eligible = out["eligible_for_default_ranking"].fillna(False).astype(bool)
    ...
    out["role_context_percentile"] = out.get("role_context_percentile", np.nan)

    if eligible.any():
        out.loc[eligible, "overall_rank"] = out.loc[eligible, "overall_impact_score"].rank(method="min", ascending=False)
        out.loc[eligible, "overall_percentile"] = _rank_pct(out.loc[eligible, "overall_impact_score"], True).values

        for _, idx in out.loc[eligible].groupby("position", dropna=False).groups.items():
            idx = list(idx)
            out.loc[idx, "position_rank"] = out.loc[idx, "overall_impact_score"].rank(method="min", ascending=False)
            out.loc[idx, "position_percentile"] = _rank_pct(out.loc[idx, "overall_impact_score"], True).values

        out.loc[eligible, "offensive_rank"] = out.loc[eligible, "offensive_score"].rank(method="min", ascending=False)

        for role_name, rank_col, score_col in [
            ("Defense", "defensive_rank", "defensive_score"),
            ("Faceoff", "faceoff_rank", "faceoff_score"),
            ("Goalie", "goalie_rank", "goalie_score"),
        ]:
            mask = eligible & out["role_group"].eq(role_name) & pd.to_numeric(out[score_col], errors="coerce").notna()
            if mask.any():
                out.loc[mask, rank_col] = out.loc[mask, score_col].rank(method="min", ascending=False)

        for _, idx in out.loc[eligible].groupby("role_group", dropna=False).groups.items():
            idx = list(idx)
    ...
    # Compatibility aliases used by current Streamlit page.
    out["v22_overall_score"] = out["overall_impact_score"]
    out["v22_overall_rank"] = out["overall_rank"]
    out["v22_overall_percentile"] = out["overall_percentile"]
    out["v22_position_rank"] = out["position_rank"]
    out["v22_position_percentile"] = out["position_percentile"]
    out["overall_score"] = out["overall_impact_score"]
    out["official_overall_score"] = out["overall_impact_score"]
    ...
    # Keep the tested public names stable. Do not drop extra source stats.
    return out.sort_values(["ranking_sort_order", "overall_rank", "overall_impact_score"], ascending=[True, True, False], na_position="last").reset_index(drop=True)
