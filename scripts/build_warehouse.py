...
def _add_player_ranking_scores(df):
    """
    Build the Colab-tested player-ranking score inputs and official score.

    This restores the tested ranking contract used by the Streamlit Player
    Rankings page:
      - component scores are percentile/peer based inside each ranking context
      - irrelevant specialist components are left blank for cleaner display
      - eligible_for_default_ranking / min_games_default aliases are preserved
      - overall_impact_score is the base impact signal expected by the app
      - overall_score / overall_rank are the official visible score/rank aliases
    """
    out = df.copy()
    ...
    needed = [
        ...
        "save_pct_calc", "two_point_shots", "two_point_goals",
        "games"
    ]
    ...
    def pct(col, higher_is_better=True):
        if col not in out.columns:
            return pd.Series(np.nan, index=out.index)
        return _rank_pct(out[col], higher_is_better=higher_is_better)

    def weighted_score(weights, fallback=50.0):
        ...
        result = score / weight_sum.replace(0, np.nan)
    ...
    # ------------------------------------------------------------------
    # Percentile-based stat signals
    # ------------------------------------------------------------------
    out["_points_score"] = pct("points_per_game", True)
    out["_scoring_points_score"] = pct("scoring_points_per_game", True)
    out["_one_point_goal_score"] = pct("one_point_goals_per_game", True)
    out["_two_point_goal_score"] = pct("two_point_goals_per_game", True)
    out["_goals_score"] = pct("goals_per_game", True)
    out["_assists_score"] = pct("assists_per_game", True)
    out["_shots_score"] = pct("shots_per_game", True)
    out["_sog_score"] = pct("shots_on_goal_per_game", True)
    out["_shot_pct_score"] = pct("shot_pct_calc", True)
    out["_touches_score"] = pct("touches_per_game", True)
    out["_passes_score"] = pct("total_passes_per_game", True)
    out["_gb_score"] = pct("ground_balls_per_game", True)
    out["_ct_score"] = pct("caused_turnovers_per_game", True)
    out["_turnover_security_score"] = pct("turnovers_per_game", False)
    out["_faceoff_pct_score"] = pct("faceoff_pct_calc", True)
    out["_faceoff_volume_score"] = pct("faceoffs_per_game", True)
    out["_faceoff_wins_score"] = pct("faceoffs_won_per_game", True)
    out["_save_pct_score"] = pct("save_pct_calc", True)
    out["_saves_score"] = pct("saves_per_game", True)
    out["_goals_against_score"] = pct("goals_against_per_game", False)
    out["_scores_against_score"] = pct("scores_against_per_game", False)

    if {"two_point_shots", "two_point_goals"}.issubset(out.columns):
        out["_two_point_goal_pct_calc"] = (
            pd.to_numeric(out["two_point_goals"], errors="coerce")
            / pd.to_numeric(out["two_point_shots"], errors="coerce").replace(0, np.nan)
        )
        out["_two_point_eff_score"] = _rank_pct(out["_two_point_goal_pct_calc"], True)
    else:
        out["_two_point_eff_score"] = np.nan

    out["one_point_goal_score"] = out["_one_point_goal_score"]
    out["two_point_goal_score"] = out["_two_point_goal_score"]

    out["goal_value_score"] = weighted_score({
        "_scoring_points_score": 0.32,
        "_points_score": 0.24,
        "_one_point_goal_score": 0.16,
        "_two_point_goal_score": 0.20,
        "_two_point_eff_score": 0.08,
    })

    out["offensive_score_raw"] = weighted_score({
        "_points_score": 0.28,
        "_scoring_points_score": 0.24,
        "_goals_score": 0.16,
        "_assists_score": 0.14,
        "_shots_score": 0.10,
    ...
    out["usage_possession_score"] = weighted_score({
        "_touches_score": 0.42,
        "_shots_score": 0.22,
        "_passes_score": 0.16,
        "_gb_score": 0.12,
    ...
    out["defensive_score_raw"] = weighted_score({
        "_ct_score": 0.48,
        "_gb_score": 0.30,
        "_turnover_security_score": 0.14,
        "_touches_score": 0.08,
    })

    out["faceoff_score_raw"] = weighted_score({
        "_faceoff_pct_score": 0.52,
        "_faceoff_wins_score": 0.28,
        "_faceoff_volume_score": 0.10,
        "_gb_score": 0.10,
    })

    out["goalie_score_raw"] = weighted_score({
        "_save_pct_score": 0.58,
        "_saves_score": 0.22,
        "_goals_against_score": 0.12,
        "_scores_against_score": 0.08,
    })

    # Public component scores: show role-relevant values only, preserving raw
    # score columns for internal calculations/debugging.
    out["offensive_score"] = np.where(out["role_group"].eq("Offense"), out["offensive_score_raw"], np.nan)
    out["defensive_score"] = np.where(out["role_group"].eq("Defense"), out["defensive_score_raw"], np.nan)
    out["faceoff_score"] = np.where(out["role_group"].eq("Faceoff"), out["faceoff_score_raw"], np.nan)
    out["goalie_score"] = np.where(out["role_group"].eq("Goalie"), out["goalie_score_raw"], np.nan)

    role_primary_map = {
        "Offense": "offensive_score_raw",
        "Defense": "defensive_score_raw",
        "Faceoff": "faceoff_score_raw",
        "Goalie": "goalie_score_raw",
    }
    ...
    for _, idx in out.groupby("role_group", dropna=False).groups.items():
        ...
        out.loc[idx, "role_reliability"] = shrink * 100
    ...
    out["_base_offense"] = weighted_score({
        "offensive_score_raw": 0.62,
        "usage_possession_score": 0.20,
        "goal_value_score": 0.18,
    })

    out["_base_defense"] = weighted_score({
        "defensive_score_raw": 0.68,
        "usage_possession_score": 0.12,
        "_gb_score": 0.14,
        "_turnover_security_score": 0.06,
    })

    out["_base_faceoff"] = weighted_score({
        "faceoff_score_raw": 0.72,
        "_gb_score": 0.18,
        "usage_possession_score": 0.10,
    })

    out["_base_goalie"] = weighted_score({
        "goalie_score_raw": 0.78,
        "_saves_score": 0.12,
        "_save_pct_score": 0.10,
    })

    out["overall_impact_score"] = np.select(
        [
        ...
        default=out["_base_offense"]
    )

    out["base_impact_score"] = out["overall_impact_score"]

    ground_ball_score = pd.to_numeric(out["_gb_score"], errors="coerce").fillna(50)
    ...
    out["v22_overall_score"] = np.select(
        ...
        [
            0.62 * out["overall_impact_score"] + 0.20 * out["role_context_value_score"] + 0.10 * out["usage_possession_score"] + 0.08 * out["goal_value_score"],
            0.60 * out["overall_impact_score"] + 0.30 * out["role_context_value_score"] + 0.10 * out["usage_possession_score"],
            0.65 * out["overall_impact_score"] + 0.25 * out["role_context_value_score"] + 0.10 * ground_ball_score,
            0.62 * out["overall_impact_score"] + 0.38 * out["role_context_value_score"],
        ],
        default=out["overall_impact_score"]
    )
    ...
    out["official_overall_score"] = out["v22_overall_score"]
    out["ranking_formula_version"] = "colab_official_player_ranking"
    out["official_score_locked"] = 1
    ...
    temp_cols = [c for c in out.columns if c.startswith("_")]
...
def _build_player_ranking_context(df, context_type, context_label, sort_order):
    ...
    eligible = (out["games"].fillna(0) >= min_games)
    out["eligible_for_default_ranking"] = eligible.astype(int)
    out["is_ranking_eligible"] = out["eligible_for_default_ranking"]
    ...
    out["v22_overall_percentile"] = np.nan
    out["v22_position_rank"] = np.nan
    out["v22_position_percentile"] = np.nan
    ...
    score_col = "v22_overall_score" if "v22_overall_score" in out.columns else "overall_score"

    if eligible.any() and score_col in out.columns:
        out.loc[eligible, "v22_overall_rank"] = out.loc[eligible, score_col].rank(method="min", ascending=False)
        out.loc[eligible, "v22_overall_percentile"] = _rank_pct(out.loc[eligible, score_col], True).values

        for _, idx in out.loc[eligible].groupby("position", dropna=False).groups.items():
            idx = list(idx)
            out.loc[idx, "v22_position_rank"] = out.loc[idx, score_col].rank(method="min", ascending=False)
            out.loc[idx, "v22_position_percentile"] = _rank_pct(out.loc[idx, score_col], True).values

        off_mask = eligible & out["role_group"].eq("Offense")
        if off_mask.any() and "offensive_score_raw" in out.columns:
            out.loc[off_mask, "offensive_rank"] = out.loc[off_mask, "offensive_score_raw"].rank(method="min", ascending=False)

        def_mask = eligible & out["role_group"].eq("Defense")
        if def_mask.any() and "defensive_score_raw" in out.columns:
            out.loc[def_mask, "defensive_rank"] = out.loc[def_mask, "defensive_score_raw"].rank(method="min", ascending=False)

        fo_mask = eligible & out["role_group"].eq("Faceoff")
        if fo_mask.any() and "faceoff_score_raw" in out.columns:
            out.loc[fo_mask, "faceoff_rank"] = out.loc[fo_mask, "faceoff_score_raw"].rank(method="min", ascending=False)

        g_mask = eligible & out["role_group"].eq("Goalie")
        if g_mask.any() and "goalie_score_raw" in out.columns:
            out.loc[g_mask, "goalie_rank"] = out.loc[g_mask, "goalie_score_raw"].rank(method="min", ascending=False)

    out["overall_rank"] = out["v22_overall_rank"]
    out["overall_percentile"] = out["v22_overall_percentile"]
    out["position_rank"] = out["v22_position_rank"]
    out["position_percentile"] = out["v22_position_percentile"]

    if "overall_score" not in out.columns and score_col in out.columns:
        out["overall_score"] = out[score_col]

    if "usage_score" not in out.columns and "usage_possession_score" in out.columns:
        out["usage_score"] = out["usage_possession_score"]
    ...
    score_cols = [
        ...
        if c.endswith("_score") or c.endswith("_percentile") or c.endswith("_rank")
        or c in ["overall_impact_score", "base_impact_score", "usage_possession_score", "usage_score"]
    ]
    ...
    return out.sort_values(
        ["ranking_sort_order", "overall_rank", "full_name"],
        na_position="last"
    ).reset_index(drop=True)
