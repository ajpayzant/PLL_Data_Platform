...
def _add_player_ranking_scores(df):
    ...
    - scoring_value_score captures direct scoring and 1PT/2PT goal value;
    - playmaking_value_score captures assists, creation efficiency, passing, and security;
    ...
    # Component scores.
    # ------------------------------------------------------------------
    # Scoring Value replaces the older Goal Value label. It captures direct
    # scoring, scoring points, 1PT/2PT goal value, and scoring efficiency.
    ...
    # Legacy alias retained so older audit/app code does not break, but all
    # final display labels should use Scoring Value.
    out["goal_value_score"] = out["scoring_value_score"]

    # Playmaking Value rewards creation that may not show up as pure goal
    # volume: assists, assists/touch, points/touch, passing involvement, and
    # turnover security. This helps creator-heavy attackmen and midfielders
    # without artificially boosting them.
    ...
    out["ranking_formula_version"] = "official_player_ranking_scoring_playmaking"
