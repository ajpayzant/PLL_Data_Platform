...
def _score_metric(series, higher_is_better=True):
    """
    Context-local metric score used by the official player ranking mart.

    This matches the final tested ranking idea more closely than pure min-max:
    percentile keeps the ranking intuitive, while min-max keeps true separation
    for players who are materially ahead of their peers.
    """
    s = pd.to_numeric(series, errors="coerce")
    ...
    return (0.65 * pct.fillna(50) + 0.35 * mm.fillna(50)).clip(0, 100).where(s.notna(), np.nan)
...
def _role_metric_score(out, col, higher_is_better=True, role_col="role_group"):
    """
    Score a metric within role groups rather than across all players.

    This is the key correction versus the broken GitHub output: faceoff,
    defensive, goalie, and usage values must be evaluated against relevant
    peers, not against every player in the league. Otherwise FO/G/D specialists
    get inflated or crushed simply because most players have zeros/missing data.
    """
    if col not in out.columns:
    ...
    for _, idx in out.groupby(role_col, dropna=False).groups.items():
        idx = list(idx)
        result.loc[idx] = _score_metric(s.loc[idx], higher_is_better=higher_is_better).values
...
def _add_player_ranking_scores(df):
    """
    Build the official player ranking mart using the same final-test logic:
      1. create role-aware component scores for offense, usage, defense,
         faceoff, goalie, 1PT/2PT scoring value, and ground-ball value;
      2. create role context from role score + role percentile + peer separation;
      3. create the official overall score once in the warehouse;
      4. expose clean final names only: overall_score, overall_rank,
         position_rank, role_context_value_score, etc.

    No v22 fields are created here. Streamlit displays these fields directly.
    """
    out = df.copy()
    ...
    out["turnovers_per_touch"] = np.where(
        pd.to_numeric(out["touches_per_game"], errors="coerce") > 0,
        pd.to_numeric(out["turnovers_per_game"], errors="coerce") / pd.to_numeric(out["touches_per_game"], errors="coerce"),
        np.nan,
    )
    out["goals_per_shot"] = np.where(
        pd.to_numeric(out["shots_per_game"], errors="coerce") > 0,
        pd.to_numeric(out["goals_per_game"], errors="coerce") / pd.to_numeric(out["shots_per_game"], errors="coerce"),
        np.nan,
    )
    out["sog_rate_for_ranking"] = np.where(
        pd.to_numeric(out["shots_per_game"], errors="coerce") > 0,
        pd.to_numeric(out["shots_on_goal_per_game"], errors="coerce") / pd.to_numeric(out["shots_per_game"], errors="coerce"),
        np.nan,
    )
    ...
    if {"two_point_shots", "two_point_goals"}.issubset(out.columns):
        ...
        out["two_point_goal_pct_calc"] = np.nan

    # ------------------------------------------------------------------
    # Role-aware metric scores. These are evaluated within comparable peer
    # groups to prevent specialist score distortion.
    # ------------------------------------------------------------------
    ...
    out["touches_score"] = _role_metric_score(out, "touches_per_game", True)
    out["passes_score"] = _role_metric_score(out, "total_passes_per_game", True)
    out["ground_ball_score"] = _role_metric_score(out, "ground_balls_per_game", True)
    out["ct_score"] = _role_metric_score(out, "caused_turnovers_per_game", True)
    out["turnover_security_score"] = _role_metric_score(out, "turnovers_per_touch", False)
    ...
    out["offensive_score_raw"] = _weighted_available_score(out, {
        "points_score": 0.28,
    ...
    out["defensive_score_raw"] = _weighted_available_score(out, {
        "ct_score": 0.50,
    ...
    out["faceoff_score_raw"] = _weighted_available_score(out, {
        "faceoff_pct_score": 0.50,
        "faceoff_wins_score": 0.26,
        "faceoff_volume_score": 0.10,
        "ground_ball_score": 0.14,
    }, fallback=50)

    out["goalie_score_raw"] = _weighted_available_score(out, {
        "save_pct_score": 0.56,
        "saves_score": 0.24,
    ...
    # Public component scores show only role-relevant values.
    ...
    # Base impact signal, retained separately for interpretation.
    out["base_impact_score"] = np.select(
        ...
        [
            ...
            _weighted_available_score(out, {
                ...
                "ground_ball_score": 0.14,
                "turnover_security_score": 0.06,
            }, fallback=50),
            _weighted_available_score(out, {
                "faceoff_score_raw": 0.62,
                "ground_ball_score": 0.25,
                "usage_possession_score": 0.13,
            }, fallback=50),
            _weighted_available_score(out, {
                "goalie_score_raw": 0.72,
                "save_pct_score": 0.16,
                "saves_score": 0.12,
            }, fallback=50),
        ],
    ...
    # Official overall score: same weights as the final tested ranking page,
    # but calculated once here in the warehouse.
    ...
    # For audit compatibility with the Colab test export, keep this aligned with
    # the visible official score while preserving base_impact_score separately.
    out["overall_impact_score"] = out["overall_score"]
    out["official_overall_score"] = out["overall_score"]
    out["ranking_formula_version"] = "official_player_ranking"
